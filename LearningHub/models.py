...
# Assuming you have a Section and Course model defined
...
class Lecture(models.Model):
    ...
    image = models.ImageField(upload_to='lecture_images/', blank=True, null=True)  # New field for image
    ...
    def __str__(self):
        return (
            f"ID: {self.id} - Title: {self.title} - Description: {self.description} "
            f"- Image: {self.image} - Video URL: {self.video_url} - Section: {self.section} "
            f"- Course: {self.course} - Lecture Slug: {self.lecture_slug} - Lecture Type: {self.lecture_type}"
        )
